...
bool CPoint3d::operator < (const GeomUtils::CPoint3d& rhs) const
{
  if (!AreEqual(x(), rhs.x())) return x() < rhs.x();
  if (!AreEqual(y(), rhs.y())) return y() < rhs.y();
  return z() < rhs.z();
}
